...
# Source: https://github.com/sheynkman-lab/Long-Read-Proteogenomics/blob/460d53ae716419438b31a3a8052f3fce2e6175ce/modules/protein_classification/src/protein_classification.py
...
def classify_protein_splice_fsm(row):
    if (
        row.pr_splice_cat == "full-splice_match"
        and row.pr_splice_subcat == "multi-exon"
    ):
        ...
        ## if nterm and cterm do not match
            ...
            and row.pr_nterm_gene_diff == 0
            and row.pr_cterm_gene_diff == 0
        ):
...
def classify_protein_splice_ism(row):
    if (
        row.pr_splice_cat == "incomplete-splice_match"
        and "mono-exon" not in row.pr_splice_subcat
    ):
            ...
            if row.pr_nterm_gene_diff == 0:
                ...
                if row.pr_nhang <= 0:
                    ...
                    if row.utr_cat == "subset":
                        ...
                        raise Exception(
                            "Invalid utr_cat - needs to be subset or unique"
                        )
                elif row.pr_nhang > 0:
                    return "pNNC,novel_nterm_known_splice_known_cterm"
...
def classify_protein_splice_nic(row):
    if (
        row.pr_splice_cat == "novel_in_catalog"
        and "mono-exon" not in row.pr_splice_subcat
    ):
        if row.pr_nterm_gene_diff == 0 and row.pr_cterm_gene_diff == 0:
            return "pNIC,known_nterm_combo_splice_known_cterm"
...
def classify_protein_splice_monoexon(row):
    if (
        row.pr_splice_subcat == "mono-exon"
        or row.pr_splice_subcat == "mono-exon_by_intron_retention"
    ):
        if (
            row.pr_splice_cat == "full-splice_match"
            and row.pr_nterm_diff == 0
            and row.pr_cterm_diff == 0
        ):
...
def main():
    ...
    parser.add_argument(
        ...
        action="store",
        dest="sqanti_protein",
        help="sqanti protein classification file",
    )
    ...
    protein_classification["protein_classification"] = protein_classification.apply(
        classify_protein, axis=1
    )
    cats = protein_classification["protein_classification"].str.split(
        ",", n=1, expand=True
    )
    protein_classification["protein_classification_base"] = cats[0]
    protein_classification["protein_classification_subset"] = cats[1]

    protein_classification.to_csv(
        f"{args.dest_dir}/{args.name}.protein_classification.tsv", sep="\t", index=False
    )
