...
# Modified by David (davidwissel@protonmail.com) to work in a haplotype-resolved proteogenomics Snakemake workflow
...
def reference_parser(args, genome_chroms):
    ...
    for r in genePredReader(referenceFiles):
        refDict[r.id] = r
        if r.length < args.min_ref_len and not args.is_fusion:
    ...
    if genome_chroms is not None:
        ...
        if len(diff) > 0:
            print(
                "WARNING: ref annotation contains chromosomes not in genome: {0}\n".format(
                    ",".join(diff)
                ),
                file=sys.stderr,
            )
...
class myProteinTranscripts(myQueryTranscripts):
    def __init__(
        ...
        super().__init__(
            ...
            strand=strand,
            bite=bite,
            RT_switching=RT_switching,
            canonical=canonical,
            min_cov=min_cov,
            min_cov_pos=min_cov_pos,
            min_samp_cov=min_samp_cov,
            sd=sd,
            FL=FL,
            FL_dict=FL_dict,
            nIndels=nIndels,
            nIndelsJunc=nIndelsJunc,
            proteinID=proteinID,
            ORFlen=ORFlen,
            CDS_start=CDS_start,
            CDS_end=CDS_end,
            ...
            FSM_class=FSM_class,
            percAdownTTS=percAdownTTS,
            seqAdownTTS=seqAdownTTS,
            dist_cage=dist_cage,
            within_cage=within_cage,
            dist_polya_site=dist_polya_site,
            within_polya_site=within_polya_site,
            polya_motif=polya_motif,
            polya_dist=polya_dist,
        )
    ...
    def modify(
        self, ref_transcript, ref_gene, tss_diff, tts_diff, refLen, refExons, ref
    ):
        super().modify(
                    ...
                    if len(ref.junctions) > 0:
                        for d, a in ref.junctions:
                            if trec.txStart < d < a < trec.txEnd:
                                ...
                                break
                    isoform_hit.modify(
                        "novel", ref.gene, "NA", "NA", ref.length, ref.exonCount, ref
                    )
                    get_gene_diff_tss_tts(isoform_hit)
                    return isoform_hit
        ...
        for rec in records:
            ...
            if isoform_hit.CDS_genomic_end != "NA":
                ...
                if len(rec.junctions) > 0:
                    if rec.strand == "+":
                        dist_to_last_junc = (
                            isoform_hit.CDS_genomic_end - rec.junctions[-1][0]
                        )
                    else:  # - strand
                        dist_to_last_junc = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--output_prefix", default="out", help="Output prefix (default: out"
    )
    ...
    output_filename = os.path.join(
        output_dir, output_prefix + ".sqanti_protein_classification.tsv"
    )
    ...
    SQANTIArgs = namedtuple(
        "SQANTIArgs",
        "isoform annotation dir output_prefix genename min_ref_len is_fusion corrGTF orf_tsv coverage window novel_gene_prefix",
    )
    ...
    ProteinArgs = namedtuple(
        "ProteinArgs",
        "isoform annotation dir output_prefix genename min_ref_len is_fusion corrGTF orf_tsv coverage window novel_gene_prefix",
    )
    # NOTE - liz - i need to stick with these names since they are originally in sqanti input
    cds_isoform_gff = (
        args.cds_isoform_gff
    )  # os.path.abspath(ddir + 'jurkat_cds_chr22.gff')
    cds_annotation_gtf = (
        args.cds_annotation_gtf
    )  # os.path.abspath(ddir + 'gencode_cds_chr22.gtf')
    protein_args = ProteinArgs(
        cds_isoform_gff,
        cds_annotation_gtf,
        dir=output_dir,
        output_prefix=output_prefix,
    ...
    isoforms_info_cds = isoformClassification(
        protein_args,
    ...
    FIELDNAMES = [
        ...
        "pr_num_exons",
        "is_nmd",
        "num_junc_after_stop_codon",
        "num_nt_after_stop_codon",
        "tx_5hang",
        "tx_3hang",
        "pr_nhang",
        "pr_chang",
    ...
    for pb, pr in isoforms_info_cds.items():
        ...
        transcript_match_id = tx.transcripts[
            0
        ]  # only get first transcript for overhang calc
        ...
        protein_match_id = pr.transcripts[
            0
        ]  # only get first transcript for overhang calc
