...
# Source: https://github.com/sheynkman-lab/Long-Read-Proteogenomics/blob/460d53ae716419438b31a3a8052f3fce2e6175ce/modules/5p_utr_status/src/1_get_gc_exon_and_5utr_info.py
...
for line in open(args.gencode_gtf):
    ...
    wds = line.split("\t")
    if wds[2] == "CDS":
        isoname = wds[8].split('transcript_id "')[1].split('"')[0]
...
gc_coords = defaultdict(
    lambda: defaultdict(lambda: [None, None, []])
)  # genename -> [<strand>, <exon chain string>, <exon coords as ints>]
...
for line in open(args.gencode_gtf):
    ...
    if wds[2] == "exon":
        isoname = wds[8].split('transcript_id "')[1].split('"')[0]
        genename = wds[8].split('gene_id "')[1].split('"')[0]
        if isoname in pc_isonames:
...
# make exon chains
gc_chains = defaultdict(
    lambda: defaultdict(lambda: [])
)  # gene -> isoname -> exon chain string
...
with open(
    os.path.join(odir, "gc_exon_chain_strings_for_cds_containing_transcripts.tsv"), "w"
) as ofile:
