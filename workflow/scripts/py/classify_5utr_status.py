...
# Source: https://github.com/sheynkman-lab/Long-Read-Proteogenomics/blob/460d53ae716419438b31a3a8052f3fce2e6175ce/modules/5p_utr_status/src/2_classify_5utr_status.py
...
def determine_if_5end_of_junction_chain_is_protruding_or_subset(
    strand, tss, junc_chain_5utr, gc_chain
):
...
def get_5utr_junc_chain_status(gene, strand, tss, junc_chain_5utr, gc_chains):
    ...
    for gc_chain in gc_chains[gene]:
        ...
        if junc_chain_5utr in gc_chain:
            tss_stat = determine_if_5end_of_junction_chain_is_protruding_or_subset(
                strand, tss, junc_chain_5utr, gc_chain
            )
            if tss_stat == "subset":
...
with open(os.path.join(args.odir, "pb_5utr_categories.tsv"), "w") as ofile:
    ofile.write(
        "pb\tgene\tnum_5utr_exons\tutr_exon_status\ttss_in_gc_exons\tjunc_cat\tutr_cat\n"
    )
    lens = []
    for pb, [
        gene,
        ...
        if exon_status == "monoexonic":
            ...
            junc_cat = get_5utr_junc_chain_status(
                gene, strand, tss, junc_chain_5utr, gc_chains
            )
